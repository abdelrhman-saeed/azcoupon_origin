...
@section('seo_title', str_replace('(v_date)', ucfirst(\Carbon\Carbon::parse(now()->toDateString())->translatedFormat('F Y')), $store->seo_title))
@section('description', str_replace('(v_date)', ucfirst(\Carbon\Carbon::parse(now()->toDateString())->translatedFormat('F Y')), $store->seo_description))

@section('canonical')
    <link rel="canonical" href="{{ route('stores_events.show', $store->slug) }}">
@endsection
    ...
    <div class="container">
        <div class="row">
            <div class="col-3 d-lg-none">
                <div class="shop-info-mobile d-sm-flex">
                    <p class="shop-info-mobile__title">Latest {{ $store->name }} Promo Codes and Discounts</p>
                    <a
                    class="fallback_link"
                    data-tracking-element
                    data-tracking-label="{{ $store->name }}"
                    data-id="{{ $store->id }}"
                    data-element="SPL" data-shop="{{ $store->name }}"
                    href="{{ $store->aff_link }}"
                    rel="nofollow"
                    target="_blank">
                        <img
                        alt="{{$store->image->alt}}"
                        title="{{$store->image->alt}}"
                        class="img-responsive"
                        height="80"
                        width="80"
                        src="{{$store->getStoreImage()}}?width=200&amp;height=200" />
                    </a>
  ...
  <main class="container" role="main">
    <div class="row">
        <div class="col m-">
            ...
            {{-- <div class="title-and-filters-container row">
                <h1 class="page-title col-12 my-1">
                    {!! str_replace('(v_date)', ucfirst( \Carbon\Carbon::parse(now()->toDateString())->translatedFormat('F Y') ), $store->seo_title ) !!}
                </h1>
            </div> --}}
    ...
    @foreach( $store_featured_coupons as $key => $coupon )
    <div class="container">
        <div class="row">
            <div
            data-index="{{$key}}"
            id="coupon-{{$coupon->id}}"
            class="coupon c-{{ $coupon->offer == 1 ? 'offer' : 'coupon' }} coupon--large"
            data-element="EP"
            data-coupon-id="{{$coupon->id}}"
            data-coupon-type="{{ $coupon->offer == 1 ? 'offer' : 'coupon' }}"
            data-filter-ids="shop-{{$coupon->id}}x">
                <div class="coupon__body">
{{--                    <span class="coupon__tag mt-3 ml-1">--}}
{{--                        Top Pick--}}
{{--                    </span>--}}
                    <a
                ...
                <div class="coupon__right">
                    ...
                    <div class="coupon__text" style="max-height: none;">
                                    ...
                                    <div class="copoun_terms my-1">
                                        <ul class="list-group-flush bg-transparent">
                                            @foreach($coupon->couponterms as $couponterm)
                                                <li class="list-group-item bg-transparent border-0 m-0 p-0" style="color:white; font-size:.9rem"> <span>-</span> {{ $couponterm->term  }} </li>
                                            @endforeach
                                        </ul>
                                    </div>
                                @endif
    ...
    <div class="card-coupon-filter remote-filter">
        <ul data-module="coupon-filter"
            ...
            <li class="coupon-filter__item all active">
                All
            </li>
            <li class="coupon-filter__item codes"  >
                Codes
            </li>
            <li class="coupon-filter__item offers">
                Offers
            </li>
        </ul>
    </div>
  ...
  <div class="coupons-list" data-element="CL" data-coupons-list >
    ...
    @foreach( $store_latest_coupons_offers as $key => $coupon )
        <div
        id="coupon-{{$coupon->id}}"
        class="coupon c-{{ $coupon->offer == 1 ? 'offer' : 'coupon' }}"
        data-coupon-item
        data-coupon-id="{{$coupon->id}}"
        data-index="{{$key}}"
        data-coupon-type="{{ $coupon->offer == 1 ? 'offer' : 'coupon' }}"
...
<div class="widget widget--summary">
    <h2>Latest {{ $store->name }} Promo Codes</h2>
    <table class="table">
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="related__col related__col--similar" data-event-category="content_related_shops">
                ...
                <h3 class="related__heading">Related shops</h3>

                <ul class="related__list">
                    @foreach($related_stores as $r_store)
                        <li class="related__item" data-shop-name="{{ $r_store->name }}">
                            <a href="{{ route('stores_events.show', $r_store->slug) }}" title="{{ $r_store->name }}" class="related__link">
                                {{ $r_store->name }}
                            </a>
                        </li>
                    @endforeach
        ...
        <div class="col-md-6">
            <div class="related__col related__col--similar" data-event-category="content_related_shops">
                ...
                <ul class="related__list">
                    @foreach($navbar_stores as $n_store)
                    <li class="related__item" data-shop-name="{{ $n_store->name }} Promo code">
                        <a href="{{ route('stores_events.show', $n_store->slug) }}" title="{{ $n_store->name }}" class="related__link">
                            {{ $n_store->name }}
                        </a>
                    </li>
                    @endforeach
...
<!-- ./related -->
    ...
    <div class="expired-coupons">
        <h4>Previous offers at {{ $store->name }}</h4>
    ...
    @include('includes.store.about_store')
</div>
      <!-- #/content -->
        <!-- .sidebar -->

        @include('includes.store.sidebar')
...
<script>
    ...
    $(function(){
        $(".card-coupon-filter .coupon-filter__item.codes").text($(".coupons-list .c-coupon").length + " Codes")
        $(".card-coupon-filter .coupon-filter__item.offers").text($(".coupons-list .c-offer").length + " Offers")
        ...
        $(".card-coupon-filter .coupon-filter__item").on("click", (e) => {
            e.preventDefault();
            $(".card-coupon-filter .coupon-filter__item").removeClass("active");
            $(e.target).addClass("active");
            if( $(e.target).hasClass('all') )
            {
                $(".coupons-list .c-coupon").fadeIn();
                $(".coupons-list .c-offer").fadeIn();
            } else if( $(e.target).hasClass('codes') )
            {
                $(".coupons-list .c-coupon").fadeIn();
                $(".coupons-list .c-offer").fadeOut();
            }else if( $(e.target).hasClass('offers') )
            {
                $(".coupons-list .c-coupon").fadeOut();
                $(".coupons-list .c-offer").fadeIn();
            }
        });
        ...
        $("input[type='radio']").click(function(){
            ...
            $.ajax({
               ...
               success: function(response){
                    $(".bold-text").removeClass( "d-none" );
                    $(".bold-text").text( response.message );
                    setTimeout(function(){
                        $(".bold-text").addClass( 'd-none' );
                    }, 5000);
               }
            })
        });
        ...
        $(document).on("click", ".subscribe-btn", (e) => {
            ...
            let $this = e.target;
            let email = $("input[name='subscribed_email']").val()
            ...
            $.ajax({
               ...
               success: function(response){
                   ...
                   $(".global-message").text( response.message );
                    if(response.success == 1){
                        ...
                        $(".global-message").removeClass( 'alert-info' );
                    }
                    setTimeout(function(){
