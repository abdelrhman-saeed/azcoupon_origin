...
@section("style")
    ...
    <script>
	tinymce.init({
        selector: '#widget_description', resize :true,
        ...
        <div class="page-content">
            ...
            <div class="card">
              <div class="card-body p-4">
                  <h5 class="card-title">Edit Widget</h5>
                  <hr/>
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    <div class="border border-3 p-4 rounded">
                                        ...
                                        <x-admin.form.input value="{!! old('title', $widget->title) !!}" required name='title' label='Widget Title' id='widget_title'/>
                                        
                                        <x-admin.form.textarea value="{!! str_replace('../../', '../../../', $widget->description) !!}" name='description' label='Widget Description' id='widget_description' rows="3"/>
                                        
                                        <x-admin.form.input type='number' value="{{ old('order', $widget->order) }}" name='order' label='Widget Order' id='widget_order'/>
                                        ...
                                        <div class="row mb-3">
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <div class="input-group">
                                                        <select id='related_sidebar' name='related_sidebar' class="single-select form-select select2-hidden-accessible" aria-hidden="true">
                                                            ...
                                                            <option {{ $widget->related_sidebar === 'Topcoupons' ? 'selected' : '' }} value='Topcoupons'>Top Coupons</option>
                                                            <option {{ $widget->related_sidebar === 'Expiresoon' ? 'selected' : '' }} value='Expiresoon'>Expire Soon Coupons</option>
                                                            <option {{ $widget->related_sidebar === 'Store' ? 'selected' : '' }} value='Store'>Store</option>
                                                            <option {{ $widget->related_sidebar === 'Event' ? 'selected' : '' }} value='Event'>Event</option>
                                                            <option {{ $widget->related_sidebar === 'Category' ? 'selected' : '' }} value='Category'>Category</option>
                                                            
                                                            <option {{ $widget->related_sidebar === 'Search' ? 'selected' : '' }} value='Search'>Search Result Page</option>
                                            ...
                                            <div class="col-lg-6 mt-4 related-store-col" style="{{ $widget->related_sidebar === 'Store' ? 'display:block;' : '' }}">
                                                ...
                                                <div class="input-group">
                                                    <select name='store_id' class="single-select form-select select2-hidden-accessible" aria-hidden="true">
                                                        @foreach($stores as $key => $store)
                                                            <option {{ $key == $widget->store_id ? 'selected' : '' }} value='{{ $key }}'>{{ $store }}</option>
                                                        @endforeach
                                                    </select>
                                            ...
                                            <div class="col-lg-6 mt-4 related-event-col" style="{{ $widget->related_sidebar === 'Event' ? 'display:block;' : '' }}">
                                                ...
                                                <div class="input-group">
                                                    <select name='event_id' class="single-select form-select select2-hidden-accessible" aria-hidden="true">
                                                        @foreach($events as $key => $event)
                                                            <option {{ $key == $widget->event_id ? 'selected' : '' }} value='{{ $key }}'>{{ $event }}</option>
                                                        @endforeach
                                                    </select>
                                            ...
                                            <div class="col-lg-6 mt-4 related-category-col" style="{{ $widget->related_sidebar === 'Category' ? 'display:block;' : '' }}">
                                                ...
                                                <div class="input-group">
                                                    <select name='category_id' class="single-select form-select select2-hidden-accessible" aria-hidden="true">
                                                        @foreach($categories as $key => $category)
                                                            <option {{ $key == $widget->category_id ? 'selected' : '' }} value='{{ $key }}'>{{ $category }}</option>
                                                        @endforeach
                                                    </select>
                                        ...
                                        <x-admin.form.button type='btn-primary'>Update Widget</x-admin.form.button>
                                        
                                        <a href='#' 
                                        onclick="
                                            if( confirm('You are gonna delete widget permanently, proceed ?') == true ) { event.preventDefault();document.getElementById('delete_widget_form').submit(); }" 
                                            class='btn btn-danger px-5'>Delete Widget</a>
                    ...
                    <form class='hidden' id='delete_widget_form' action="{{ route('admin.widgets.destroy', $widget) }}" method='POST'>
                        @csrf @method('DELETE')
                    </form>
                </div>
            </div>
